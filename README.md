...
git clone https://github.com/qingh097/b1k.git capx/third_party/b1k
...
source .venv-libero/bin/activate
uv run --no-sync --active capx/envs/launch.py \
    --config-path env_configs/libero/libero_spatial_0.yaml \
    --model "google/gemini-3.1-pro-preview"
...
> **Tip (BEHAVIOR):** For best performance on multi-GPU systems, run perception servers on a separate GPU from Isaac Sim (e.g. `CUDA_VISIBLE_DEVICES=0` for the eval, and pre-launch SAM3/GraspNet on GPU 1). Set `OMNI_KIT_ACCEPT_EULA=YES` and `OMNIGIBSON_HEADLESS=1` for headless evaluation.
...
| [Claude Code Integration](docs/claude-code-integration.md) | Autoresearch agent, skill library, slash commands |

### Claude Code as Autonomous Research Agent

CaP-X integrates with [Claude Code](https://docs.anthropic.com/en/docs/claude-code) as an autonomous research agent that iteratively improves task success rates. Claude Code runs evaluations, analyzes failures, modifies prompts and skills, and re-evaluates in a closed-loop hillclimbing system:
